...
// Detect Light Type
	...
	//Surface Shader Haxies:
				...
				// Unity 5 Version:
				#undef UnitySampleShadowmap
				#define UnitySampleShadowmap(coord) unitySampleShadow(coord)
			#endif
			...
			// Unity 5 Version:
			#undef UnitySampleShadowmap
			#define UnitySampleShadowmap(coord) unityCubeShadow(coord)
		#endif
	#endif	
	...
	fixed ShadowFilterPCF3x3(float3 shadowCoordDepth)
	{
		...
		for (int y = 0; y < 3; y++)
		{
			const float offsetInner = 0.0, offsetEdge = 1.0, offsetCorner = 1.414213562373095; // 0x0, 0x1, 1x1 - 0.5
			float3 offsetLengths;
			if(y==0 || y==2)	offsetLengths = float3(offsetCorner, offsetEdge, offsetCorner);
			else				offsetLengths = float3(offsetEdge, offsetInner, offsetEdge);
	...
	fixed ShadowFilterPCF4x4(float3 shadowCoordDepth)
	{
		...
		for (int y = 0; y < 4; y++)
		{
			const float offsetInner = 0.707106781186548, offsetEdge = 1.58113883008419, offsetCorner = 2.121320343559642; //0.5x0.5, 1.5x0.5, 1.5x1.5
			float4 offsetLengths;
			if(y==0 || y==3)	offsetLengths = float4(offsetCorner, offsetEdge, offsetEdge, offsetCorner);
			else				offsetLengths = float4(offsetEdge, offsetInner, offsetInner, offsetEdge);
			
			float4 samples;
			samples.x = SOFTENER_SAMPLE_SHADOW(float3(sampleCoord + _ShadowMapTexture_TexelSize.xy * float2(-1, y - 1), SOFTENER_SAMPLE_BIAS(shadowCoordDepth.z, offsetLengths.x)));
			samples.y = SOFTENER_SAMPLE_SHADOW(float3(sampleCoord + _ShadowMapTexture_TexelSize.xy * float2( 0, y - 1), SOFTENER_SAMPLE_BIAS(shadowCoordDepth.z, offsetLengths.y)));
			samples.z = SOFTENER_SAMPLE_SHADOW(float3(sampleCoord + _ShadowMapTexture_TexelSize.xy * float2( 1, y - 1), SOFTENER_SAMPLE_BIAS(shadowCoordDepth.z, offsetLengths.z)));
			samples.w = SOFTENER_SAMPLE_SHADOW(float3(sampleCoord + _ShadowMapTexture_TexelSize.xy * float2( 2, y - 1), SOFTENER_SAMPLE_BIAS(shadowCoordDepth.z, offsetLengths.w)));
	
			fixed4 inLight = SOFTENER_4_SAMPLES_LIT(samples, SOFTENER_SAMPLE_BIAS(shadowCoordDepth.zzzz, offsetLengths));
			
			shadowTerm[y] = dot(inLight, fixed4(fracWeights.z, 1, 1, fracWeights.x));
		}        
	...
	// Broken in Unity 5
	fixed ShadowFilterPCF4x4_OPTIMIZED_FOR_CACHE(float3 shadowCoordDepth)
	{
		//4x4 isn't centered nicely :)
		shadowCoordDepth.xy -= _ShadowMapTexture_TexelSize.xy * 0.5;
		...
		for (int y = 0; y < 2; y++)
		{
			for (int x = 0; x < 2; x++)
			{
				...
				const float offsetInner = 0.707106781186548, offsetEdge = 1.58113883008419, offsetCorner = 2.121320343559642; //0.5x0.5, 1.5x0.5, 1.5x1.5
				float4 offsetLengths;
				if(x==0 && y==0)		offsetLengths = float4(offsetCorner, offsetEdge, offsetEdge, offsetInner);
				else if(x==1 && y==0)	offsetLengths = float4(offsetEdge, offsetCorner, offsetInner, offsetEdge);
				else if(x==0 && y==1)	offsetLengths = float4(offsetEdge, offsetInner, offsetCorner, offsetEdge);
				else					offsetLengths = float4(offsetInner, offsetEdge, offsetEdge, offsetCorner);
...
#else
	#undef SOFTENER_CUSTOM_FILTER
	#define SOFTENER_CUSTOM_FILTER(data) 1
#endif