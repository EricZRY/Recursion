...
// =================================== BEGIN SHADOW SOFTENER SUPPORT ===================================
#if defined (SHADOWS_DEPTH) && defined (SPOT)
#define SOFTENER_PREPASS
#define SOFTENER_SPOT
float UnitySampleShadowmap(float4 coord);
#endif
#if defined (SHADOWS_CUBE)
#define SOFTENER_PREPASS
#define SOFTENER_SPOT
float UnitySampleShadowmap(float3 coord);
#endif
// =================================== END SHADOW SOFTENER SUPPORT ===================================
...
float UnitySampleShadowmap(float4 coord)
{
	float shadow = SOFTENER_CUSTOM_FILTER((coord).xyz/(coord).w);
	return _LightShadowData.r + shadow * (1-_LightShadowData.r);
}
#endif
#ifdef SOFTENER_POINT
float UnitySampleShadowmap(float3 coord)
{
	float mydist = length(coord) * _LightPositionRange.w;
	//mydist *= 0.97; // bias
	float shadow = SOFTENER_CUSTOM_FILTER(float4(coord, mydist));
	return shadow;
}
#endif
// =================================== END SHADOW SOFTENER SUPPORT ===================================
